...
class RelatorioPainelCorrespondenteController extends Controller
{
    ...
    public function buscar(Request $request)
    {
        if (\Helper::validaData($request->dtInicio) && \Helper::validaData($request->dtFim) && strtotime(str_replace('/', '-', $request->dtInicio)) <= strtotime(str_replace('/', '-', $request->dtFim))) {
            ...
            if (!$processos->isEmpty()) {
                \Excel::store(new RelacaoProcessosExport($dados), "/correspondentes/{$this->conta}/reports/".time().$fileName.'.xlsx', 'arquivos', \Maatwebsite\Excel\Excel::XLSX);
            } else {
    ...
    {
        \File::makeDirectory(storage_path()."/arquivos/correspondentes/{$this->conta}/reports/", $mode = 0777, true, true);

        $arquivos = array();

        $files = collect(\File::allFiles(storage_path()."/arquivos/correspondentes/{$this->conta}/reports/"))
    ...
    {
        \Storage::disk('arquivos')->delete("/correspondentes/{$this->conta}/reports/".$nome);
    ...
    public function arquivo($nome)
    {
        ...
        return response()->download(storage_path("/arquivos/correspondentes/{$this->conta}/reports/".$nome));
    }
}
