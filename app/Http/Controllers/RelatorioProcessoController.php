...
class RelatorioProcessoController extends Controller
{
            ...
            if ($request->relatorio == 'para-cliente') {
                if (!empty($cliente)) {
                    ...
                    if (!$processos->isEmpty()) {
                        ...
                        \Excel::store(new ProcessoParaClienteExport($dados), "{$this->conta}/reports/processo/".time() . "_".$request->nm_cliente_cli.'.xlsx', 'arquivos', \Maatwebsite\Excel\Excel::XLSX);
                    } else {
                ...
                if (!$processos->isEmpty()) {
                    \Excel::store(new ProcessoParaTodosClientesExport($dados), "{$this->conta}/reports/processo/".time() . "_Todos_Clientes.xlsx", 'arquivos', \Maatwebsite\Excel\Excel::XLSX);
                } else {
                    Flash::error('Não há dados para os parâmetros informados!');
                }
            }
    ...
    {
        \File::makeDirectory(storage_path()."/arquivos/{$this->conta}/reports/processo", $mode = 0744, true, true);

        $arquivos = array();

        $files = collect(\File::allFiles(storage_path()."/arquivos/{$this->conta}/reports/processo"))
    ...
    {
        \Storage::disk('arquivos')->delete("{$this->conta}/reports/processo/$nome");
    ...
    public function arquivo($nome)
    {
        ...
        return response()->download(storage_path("arquivos/{$this->conta}/reports/processo/$nome"));
    }
}
