    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
            <h1 class="page-title txt-color-blueDark">
                <i class="fa-fw fa fa-group"></i> Clientes <span> > Contatos</span>
            </h1>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 boxBtnTopo">
            <a href="{{ url('cliente/'.$id.'/contato/novo') }}" class="btn btn-success pull-right header-btn"><i class="fa fa-plus fa-lg"></i> Novo</a>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="well center">
                @foreach(range('A', 'Z') as $letra) 
                    @if(!empty(session('inicial')) and session('inicial') == $letra)
                        <a class="btn btn-primary btn-xs btn_sigla" href="{{ url('cliente/'.$id.'/contato/buscar/'.$letra) }}">{{ $letra }}</a>
                    @else
                        <a class="btn btn-default btn-xs btn_sigla" href="{{ url('cliente/'.$id.'/contato/buscar/'.$letra) }}">{{ $letra }}</a>
                    @endif
                @endforeach
            </div>
