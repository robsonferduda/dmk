<table>
    ...
    <tr>
        ...
        <th style="background-color:#D99594;height:50px;border:1px hair #000000;text-align:center;vertical-align:center">DATA DA SOLICITAÇÃO</th>
    ...
    <tbody>
        ...
        <tr>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->advogadoSolicitante ? $dado->advogadoSolicitante->nm_contato_cot : ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->dt_solicitacao_pro ? date('d/m/Y', strtotime($dado->dt_solicitacao_pro)) : ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->dt_prazo_fatal_pro ? date('d/m/Y', strtotime($dado->dt_prazo_fatal_pro)) : ' ' }} {{ $dado->hr_audiencia_pro ? date('H:i', strtotime($dado->hr_audiencia_pro)) : ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->nm_autor_pro ?? ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->nm_reu_pro ?? ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->nu_processo_pro ?? ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->vara ? $dado->vara->nm_vara_var : ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->cidade ? $dado->cidade->nm_cidade_cde : ' ' }}-{{ $dado->cidade && $dado->cidade->estado ? $dado->cidade->estado->sg_estado_est : ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->honorario && $dado->honorario->tipoServico ? $dado->honorario->tipoServico->nm_tipo_servico_tse : ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->nu_acompanhamento_pro ?? ' ' }}</td>
            <td style="border:1px hair #000000;vertical-align:center">{{ $dado->honorario ? $dado->honorario->vl_taxa_honorario_cliente_pth : 0 }}</td>
            ...
            @foreach($dados['despesas'] as $despesa)
                @php
                    ...
                    if ($item) {
                        $despesaValor = $item->pivot->vl_processo_despesa_pde;
                        $totalDespesas += $despesaValor;
                    }
                @endphp
                <td style="border:1px hair #000000;vertical-align:center">{{ $despesaValor }}</td>
            @endforeach
            <td style="border:1px hair #000000;vertical-align:center">
                @php
                    ...
                    $totalLinha = (float) $totalDespesas + $taxaHonorario;
                    $total += $totalLinha;
                @endphp
                {{ $totalLinha }}
            </td>
        ...
        @endforeach
        <tr>
            <td colspan="3" style="background-color:#969696;height:50px;border-bottom:1px hair #000000;text-align:center;vertical-align:center;font-weight:bold;font-size:12px">
